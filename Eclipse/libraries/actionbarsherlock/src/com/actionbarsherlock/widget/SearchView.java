 ...
 *
 * <p>When the SearchView is used in an ActionBar as an action view for a collapsible menu item, it
 ...
 *
 * <p>If you want the search field to always be visible, then call setIconifiedByDefault(false).
 * <div class="special reference">
 *
 * <h3>Developer Guides</h3>
 *
 * <p>For information about using {@code SearchView}, read the <a
 * href="{@docRoot}guide/topics/search/index.html">Search</a> developer guide. </div>
 *
...
public class SearchView extends LinearLayout implements CollapsibleActionView {
  ...
  public interface OnCloseListener {
     ...
     *
     * @return true if the listener wants to override the default behavior of clearing the text
     *     field and dismissing it, false otherwise.
     */
    boolean onClose();
  }

  /**
   * Callback interface for selection events on suggestions. These callbacks are only relevant when
   * a SearchableInfo has been specified by {@link #setSearchableInfo}.
   */
  public interface OnSuggestionListener {
     ...
     *     possibly rewriting the query based on the selected item, false otherwise.
     */
     ...
     * @return true if the listener handles the event and wants to override the default behavior of
     *     launching any intent or submitting a search query specified on that item. Return false
     *     otherwise.
     */
  ...
  /**
   * Sets the SearchableInfo for this SearchView. Properties in the SearchableInfo are used to
   * display labels, hints, suggestions, create intents for launching search results screens and
   * controlling other affordances such as a voice button.
   *
   ...
   *
   * @param listener the listener object that receives callbacks when the user performs actions in
  ...
  /**
   * Sets a listener to inform when the search button is pressed. This is only relevant when the
   * text field is not visible by default. Calling {@link #setIconified setIconified(false)} can
   * also cause this listener to be informed.
   *
   * @param listener the listener to inform when the search button is clicked or the text field is
   ...
   *
   * @param query the query string. This replaces any query text already present in the text field.
   * @param submit whether to submit the query right now or only update the contents of text field.
   */
  ...
  /**
   * Sets the hint text to display in the query text field. This overrides any hint specified in the
   * SearchableInfo.
   *
   ...
   * by default and expands to show the text field and other buttons when pressed. Also, if the
   * default state is iconified, then it collapses to that state when the close button is pressed.
  ...
  /**
   * Iconifies or expands the SearchView. Any query text is cleared when iconified. This is a
   * temporary state and does not override the default iconified state set by {@link
   * #setIconifiedByDefault(boolean)}. If the default state is iconified, then a false here will
   * only be valid until the user closes the field. And if the default state is expanded, then a
   * true here will only clear the text field and not close it.
   *
   * @param iconify a true value will collapse the SearchView to an icon, while a false will expand
   *     it.
   */
   ...
   *
   * @return true if the SearchView is currently iconified, false if the search field is fully
   *     visible.
   */
  ...
  /**
   * Enables showing a submit button when the query is non-empty. In cases where the SearchView is
   * being used to filter the contents of the current activity and doesn't launch a separate results
   * activity, then the submit button should be disabled.
   *
   * @param enabled true to show a submit button for submitting queries, false if a submit button is
   *     not required.
   */
   ...
   * with {@link #setSearchableInfo(SearchableInfo)} and not when using a custom adapter.
   *
  ...
  /**
   * You can set a custom adapter if you wish. Otherwise the default adapter is used to display the
   * suggestions from the suggestions provider associated with the SearchableInfo.
   *
  ...
  /**
   * Gets the specified maximum width in pixels, if set. Returns zero if no maximum width was
   * specified.
   *
   ...
   *
   * @param keyCode This is the keycode of the typed key, and is the same value as found in the
   *     KeyEvent parameter.
  ...
  /**
   * React to the user typing "enter" or other hardwired keys while typing in the search box. This
   * handles these special keys while the edit box has focus.
   */
          ...
          if (!mQueryTextView.isEmpty() && KeyEventCompat.hasNoModifiers(event)) {
            if (event.getAction() == KeyEvent.ACTION_UP) {
              if (keyCode == KeyEvent.KEYCODE_ENTER) {
                ...
                launchQuerySearch(
                    KeyEvent.KEYCODE_UNKNOWN, null, mQueryTextView.getText().toString());
                return true;
              }
            }
  ...
  /**
   * React to the user typing while in the suggestions list. First, check for action keys. If not
   * handled, try refocusing regular characters into the EditText.
   */
  ...
  /**
   * For a given suggestion and a given cursor row, get the action message. If not provided by the
   * specific row/column, also check for a single definition (for the action key).
   *
   * @param c The cursor providing suggestions
   * @param actionKey The actionkey record being examined
   * @return Returns a string, or null if no action key message for this suggestion
   */
  // TODO private static String getActionKeyMessage(Cursor c, SearchableInfo.ActionKeyInfo
  // actionKey) {
  ...
  private void updateSearchAutoComplete() {
    ...
    if (mSearchable.getSuggestAuthority() != null) {
      mSuggestionsAdapter =
          new SuggestionsAdapter(getContext(), this, mSearchable, mOutsideDrawablesCache);
      mQueryTextView.setAdapter(mSuggestionsAdapter);
      ((SuggestionsAdapter) mSuggestionsAdapter)
          .setQueryRefinement(
              mQueryRefinement
                  ? SuggestionsAdapter.REFINE_ALL
                  : SuggestionsAdapter.REFINE_BY_ENTRY);
    }
  }

  /**
   * Update the visibility of the voice button. There are actually two voice search modes, either of
   * which will activate the button.
   *
   * @param empty whether the search query text field is empty. If it is, then the other criteria
   *     apply to make the voice button visible.
   */
  ...
  private void adjustDropDownSizeAndPosition() {
    if (mDropDownAnchor.getWidth() > 1) {
      ...
      mQueryTextView.setDropDownHorizontalOffset(
          -(dropDownPadding.left + iconOffset) + anchorPadding);
      mQueryTextView.setDropDownWidth(
          mDropDownAnchor.getWidth()
              + dropDownPadding.left
              + dropDownPadding.right
              + iconOffset
              - (anchorPadding));
    }
  }
   ...
   * @param actionKey The key code of the action key that was pressed, or {@link
  ...
  private Intent createIntent(
      String action, Uri data, String extraData, String query, int actionKey, String actionMsg) {
  ...
  /**
   * Create and return an Intent that can launch the voice search activity, perform a specific voice
   * transcription, and forward the results to the searchable activity.
   *
  ...
  private Intent createVoiceAppSearchIntent(Intent baseIntent, SearchableInfo searchable) {
    ...
    PendingIntent pending =
        PendingIntent.getActivity(getContext(), 0, queryIntent, PendingIntent.FLAG_ONE_SHOT);
  ...
  /**
   * When a particular suggestion has been selected, perform the various lookups required to use the
   * suggestion. This includes checking the cursor for suggestion-specific data, and/or falling back
   * to the XML for defaults; It also creates REST style Uri data when the suggestion includes a
   * data id.
   *
  ...
  public static class SearchAutoComplete extends AutoCompleteTextView {
    ...
    /**
     * We override this method to avoid an extra onItemClick being called on the drop-down's
     * OnItemClickListener by {@link AutoCompleteTextView#onKeyUp(int, KeyEvent)} when an item is
  ...
  private static void showSoftInputUnchecked(View view, InputMethodManager imm, int flags) {
    try {
      Method method =
          imm.getClass().getMethod("showSoftInputUnchecked", int.class, ResultReceiver.class);
      method.setAccessible(true);
  ...
  private static void setText(AutoCompleteTextView view, CharSequence text, boolean filter) {
    try {
      Method method =
          AutoCompleteTextView.class.getMethod("setText", CharSequence.class, boolean.class);
      method.setAccessible(true);
