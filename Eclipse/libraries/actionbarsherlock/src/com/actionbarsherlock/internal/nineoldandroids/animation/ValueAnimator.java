...
/**
 * This class provides a simple timing engine for running animations which calculate animated values
 * and set them on target objects.
 *
 * <p>There is a single timing pulse that all animations use. It runs in a custom handler to ensure
 * that property changes happen on the UI thread.
 *
 * <p>By default, ValueAnimator uses non-linear time interpolation, via the {@link
 * AccelerateDecelerateInterpolator} class, which accelerates into and decelerates out of an
 * animation. This behavior can be changed by calling {@link
...
public class ValueAnimator extends Animator {
  ...
  /**
   * Messages sent to timing handler: START is sent when an animation first begins, FRAME is sent by
   * the handler to itself to process the next animation frame
   */
  ...
  /**
   * Internal per-thread collections used to avoid set collisions as animations start and end while
   * being processed.
   */
  ...
  /**
   * Used to indicate whether the animation is currently playing in reverse. This causes the elapsed
   * fraction to be inverted to calculate the appropriate values.
   */
  ...
  /**
   * Tracks the time at which the animation began playing through its startDelay. This is different
   * from the mStartTime variable, which is used to track when the animation became active (which is
   * when the startDelay expired and the animation was added to the active animations list).
   */
  ...
  /**
   * Additional playing state to indicate whether an animator has been start()'d. There is some lag
   * between a call to start() and the first animation frame. We should still note that the
  ...
  /**
   * The type of repetition that will occur when repeatMode is nonzero. RESTART means the animation
   * will start from the beginning on every new cycle. REVERSE means the animation will reverse
   * directions on each iteration.
   */
  private int mRepeatMode = RESTART;

  /**
   * The time interpolator to be used. The elapsed fraction of the animation will be passed through
   * this interpolator to calculate the interpolated fraction, which is then used to calculate the
   * animated values.
   */
  ...
  /**
   * A hashmap of the PropertyValuesHolder objects. This map is used to lookup animated values by
   * property name during calls to getAnimatedValue(String).
   */
  ...
  /**
   * Creates a new ValueAnimator object. This default constructor is primarily for use internally;
   * the factory methods which take parameters are more generally useful.
   */
  ...
  /**
   * Sets int values that will be animated between. A single value implies that that value is the
   * one being animated to. However, this is not typically useful in a ValueAnimator object because
   * there is no way for the object to determine the starting value for the animation (unlike
   * ObjectAnimator, which can derive that value from the target object and property being
   * animated). Therefore, there should typically be two or more values.
   *
   * <p>If there are already multiple sets of values defined for this ValueAnimator via more than
   * one PropertyValuesHolder object, this method will set the values for the first of those
   * objects.
   *
   * @param values A set of values that the animation will animate between over time.
   */
  ...
  /**
   * Sets float values that will be animated between. A single value implies that that value is the
   * one being animated to. However, this is not typically useful in a ValueAnimator object because
   * there is no way for the object to determine the starting value for the animation (unlike
   * ObjectAnimator, which can derive that value from the target object and property being
   * animated). Therefore, there should typically be two or more values.
  ...
  /**
   * Sets the values to animate between for this animation. A single value implies that that value
   * is the one being animated to. However, this is not typically useful in a ValueAnimator object
   * because there is no way for the object to determine the starting value for the animation
   * (unlike ObjectAnimator, which can derive that value from the target object and property being
   * animated). Therefore, there should typically be two or more values.
   *
   * <p>If there are already multiple sets of values defined for this ValueAnimator via more than
   * one PropertyValuesHolder object, this method will set the values for the first of those
   ...
   * between these value objects. ValueAnimator only knows how to interpolate between the primitive
   * types specified in the other setValues() methods.
   *
    ...
    if (mValues == null || mValues.length == 0) {
      setValues(
          new PropertyValuesHolder[] {
            PropertyValuesHolder.ofObject("", (TypeEvaluator) null, values)
          });
    } else {
      PropertyValuesHolder valuesHolder = mValues[0];
  ...
  /**
   * Sets the values, per property, being animated between. This function is called internally by
   * the constructors of ValueAnimator that take a list of values. But an ValueAnimator can be
   * constructed without values and this method can be called to set the values manually instead.
   *
   ...
   * PropertyValuesHolder objects, even if the ValueAnimator was created with a simple list of value
   * objects instead.
   *
   * @return PropertyValuesHolder[] An array of PropertyValuesHolder objects which hold the values,
   *     per property, that define the animation.
   */
   ...
   * delay ends. It takes care of the final initialization steps for the animation.
   *
   * <p>Overrides of this method should call the superclass method to ensure that internal
   * mechanisms for the animation are set up correctly.
   */
  void initAnimation() {
    if (!mInitialized) {
   ...
   *     ValueAnimator.ofInt(0, 10).setDuration(500).start()</code>.
   */
   ...
   * purpose for this read-only property is to retrieve the value from the <code>ValueAnimator
   * </code> during a call to {@link AnimatorUpdateListener#onAnimationUpdate(ValueAnimator)}, which
  ...
  /**
   * The most recent value calculated by this <code>ValueAnimator</code> for <code>propertyName
   * </code>. The main purpose for this read-only property is to retrieve the value from the <code>
   * ValueAnimator</code> during a call to {@link
   * AnimatorUpdateListener#onAnimationUpdate(ValueAnimator)}, which is called during each animation
   * frame, immediately after the value is calculated.
   *
   * @return animatedValue The value most recently calculated for the named property by this <code>
   *     ValueAnimator</code>.
   */
   ...
   *
   * @param listener the listener to be removed from the current set of update listeners for this
   *     animation.
   */
   ...
   * acceleration and deceleration. The default value is {@link
   * android.view.animation.AccelerateDecelerateInterpolator}
   *
   * @param value the interpolator to be used by this animation. A value of <code>null</code> will
  ...
  /**
   * The type evaluator to be used when calculating the animated values of this animation. The
   * system will automatically assign a float or int evaluator based on the type of <code>startValue
   * </code> and <code>endValue</code> in the constructor. But if these values are not one of these
   * primitive types, or if different evaluation is desired (such as is necessary with int values
   * that represent colors), a custom evaluator needs to be assigned. For example, when running an
   * animation on color values, the {@link ArgbEvaluator} should be used to get correct RGB color
   * interpolation.
   *
   * <p>If this ValueAnimator has only one set of values being animated between, this evaluator will
   * be used for that set. If there are several sets of values being animated, which is the case if
   * PropertyValuesHOlder objects were set on the ValueAnimator, then the evaluator is assigned just
   * to the first PropertyValuesHolder object.
   *
   ...
   * set for the current animation; future playing of the animation will use the default behavior of
   * playing forward.
   */
  public void reverse() {
   ...
   *
   * @param currentTime The current animation time, used to calculate whether the animation has
   *     exceeded its <code>startDelay</code> and should be started.
  ...
  /**
   * Returns the current animation fraction, which is the elapsed/interpolated fraction used in the
   * most recent frame update on the animation.
   *
  ...
  /**
   * Implementors of this interface can add themselves as update listeners to an <code>ValueAnimator
   * </code> instance to receive callbacks on every animation frame, after the current frame's
   * values have been calculated for that <code>ValueAnimator</code>.
   */
   ...
   *
   * <p>Used by StrictMode internally to annotate violations. Only called on the main thread.
   *
   * @hide
   */
  ...
  /**
   * Clear all animations on this thread, without canceling or ending them. This should be used with
   * caution.
   *
   * @hide
   */
