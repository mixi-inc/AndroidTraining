...
## 実習・課題
実習、課題で使用する各プロジェクトをAndroidStudioで読み込む方法は[アプリのレイアウト作成](http://mixi-inc.github.io/AndroidTraining/fundamentals/2.01.create-layout.html#実習)を参照してください

### 実習
この実習で使用するプロジェクトは1-2-ControllerPracticeです。
...
2. Fragment 1で作った Fragment を、Activity 1 で作成した Activity に組み込んでください。

### 課題
この実習で使用するプロジェクトは1-2-ControllerAssignmentです。
...
2. Controller2Activity で、状態管理を適切に行なってください
3. `activity_controller3.xml`から、Fragment に切り出すものを適宜選択し、Fragment 化してください。
4. 課題 3 で切り出した Fragment を用いて、新しい画面を作成してください
5. Controller4Activity で発生しているメモリリークを特定し、解消してください。
